...
class ConnectionManager:
    ...
    Pattern: One manager instance shared across all WebSocket endpoints.
    Thread-safe using asyncio (single event loop).
    """
    ...
    async def connect(self, websocket: WebSocket) -> str:
        ...
        Why UUID: Ensures globally unique session IDs even across server restarts.
        """
    ...
    async def disconnect(self, session_id: str):
        ...
        Best Practice: Always clean up resources to prevent memory leaks.
        """
    ...
    async def send_message(self, session_id: str, message: dict):
        ...
        Args:
            session_id: Target connection
            message: Dict to send as JSON
        
        Error Prevention: Check if connection still active before sending.
        """
        if session_id not in self.active_connections:
            logger.warning(
                f"Attempted to send to disconnected session",
    ...
    async def broadcast(self, message: dict, exclude: Optional[str] = None):
        ...
        Args:
            ...
            exclude: Optional session_id to skip
        
        Use Case: Server announcements, system status updates
        """
    ...
    async def send_text(self, session_id: str, text: str):
        """Send plain text message (not JSON)."""
        if session_id in self.active_connections:
            try:
    ...
    async def send_bytes(self, session_id: str, data: bytes):
        """Send binary data (for audio streaming)."""
        if session_id in self.active_connections:
            try:
    ...
    async def keepalive(self, session_id: str, interval: int = 30):
        ...
        Why: Some proxies/load balancers close idle connections.
        Heartbeat prevents unexpected disconnects.
        
        Args:
            session_id: Connection to keep alive
